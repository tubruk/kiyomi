...
// Synchronously fetches the chapter page list from the provider, persists pages.json
// to the library, and enqueues a pull_page job per page for image download.
func (h *Handler) pullChapter(c echo.Context) error {
	...
	if chMeta, err := h.lib.GetChapter(mangaID, providerID, chapterID); err == nil && chMeta.Content != nil && chMeta.Content.ChapterRef != "" {
		remoteChapterID = chMeta.Content.ChapterRef
	}

	if mangaMeta, err := h.lib.GetManga(mangaID); err == nil && mangaMeta.Content != nil && mangaMeta.Content.ProviderMangaID != "" {
	...
	if err := h.lib.SaveChapterPages(mangaID, providerID, chapterID, pageItems); err != nil {
		c.Set("handler_error", err.Error())
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err.Error()})
	}
	...
	for _, p := range pages {
		jobID, enqueueErr := h.enqueuePagePull(c.Request().Context(), mangaID, providerID, chapterID, p.Index, p.URL)
		if enqueueErr != nil {
			c.Set("handler_error", enqueueErr.Error())
			return c.JSON(http.StatusInternalServerError, echo.Map{"error": enqueueErr.Error()})
		}
		if jobID != "" {
...
// enqueuePagePull persists and enqueues a pull_page job. Returns the job ID or empty string if queue unavailable.
func (h *Handler) enqueuePagePull(ctx context.Context, mangaID, providerID, chapterID string, pageIndex int, pageURL string) (string, error) {
	if h.jobStore == nil || h.enqueuer == nil {
		return "", nil
	}

	payloadBytes, err := json.Marshal(queue.PullPagePayload{
		MangaID:    mangaID,
		ProviderID: providerID,
		ChapterID:  chapterID,
		PageIndex:  pageIndex,
	...
	if err != nil {
		return "", fmt.Errorf("marshal pull_page payload: %w", err)
	}
	...
	if err := h.jobStore.CreateJob(ctx, job); err != nil {
		return "", fmt.Errorf("persist pull_page job: %w", err)
	}
	if err := h.enqueuer.Enqueue(ctx, job); err != nil {
		return "", fmt.Errorf("enqueue pull_page job: %w", err)
	}
	return job.ID, nil
}
