...
func TestRequestBuilder_InitializesJar(t *testing.T) {
	client := &http.Client{Timeout: 5 * time.Second}
	if client.Jar != nil {
		t.Fatal("expected client.Jar to be nil initially")
	}
	b := upstream.NewRequestBuilder(nil, nil, client)
	if b.HTTPClient().Jar == nil {
		t.Errorf("expected RequestBuilder to initialize httpClient.Jar")
	}
	if client.Jar == nil {
		t.Errorf("expected client.Jar to be set")
	}
}

func TestRequestBuilder_ConcurrentBuildRequest(t *testing.T) {
	fpStore := fingerprint.NewMemoryStore()
	_ = fpStore.Set("prov-concurrent", fingerprint.Profile{
		Cookies: map[string]string{
			"https://example.com": "session_id=concur123; user=bob",
		},
	})
	client := &http.Client{Timeout: 5 * time.Second}
	b := upstream.NewRequestBuilder(fpStore, nil, client)
	...
	for i := 0; i < 20; i++ {
		...
		go func() {
			defer wg.Done()
			req, err := b.BuildRequest(context.Background(), "https://example.com/page/1.jpg", "prov-concurrent", "")
			if err != nil {
				t.Errorf("BuildRequest failed: %v", err)
				return
			}
			if req == nil {
				t.Errorf("expected non-nil req")
			}
		}()
	}
	wg.Wait()
