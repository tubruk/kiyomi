...
type PullHandlers struct {
	...
	mangaLocksMu sync.Mutex
	mangaLocks   map[string]*sync.Mutex
}
...
func NewPullHandlers(lib *library.Library, reg *provider.Registry, httpClient *http.Client, fpStore fingerprint.Store, enqueuer Enqueuer, jobStore JobStore) *PullHandlers {
	...
	return &PullHandlers{
		...
		jobStore:   jobStore,
		logger:     slog.Default(),
		mangaLocks: make(map[string]*sync.Mutex),
	}
}
...
// lockManga returns a per-manga mutex, creating one if needed. Callers must
// hold the returned mutex while running fan-out operations to prevent
// duplicate child jobs from concurrent pull_manga invocations.
func (d *PullHandlers) lockManga(mangaID string) *sync.Mutex {
	d.mangaLocksMu.Lock()
	defer d.mangaLocksMu.Unlock()
	if m, ok := d.mangaLocks[mangaID]; ok {
		return m
	}
	m := &sync.Mutex{}
	d.mangaLocks[mangaID] = m
	return m
}
...
func (d *PullHandlers) HandlePullManga(ctx context.Context, job *Job) error {
	...
	for _, ch := range chapters {
		...
		if err := d.enqueueChild(ctx, JobTypePullChapter, PullChapterPayload{
			MangaID:    p.MangaID,
			ProviderID: p.ProviderID,
			ChapterID:  ch.ID,
	...
	if p.CoverURL != "" {
		...
		if claimed {
			if err := d.enqueueChild(ctx, JobTypePullCover, PullCoverPayload{
				MangaID:    p.MangaID,
				CoverURL:   p.CoverURL,
				ProviderID: p.ProviderID,
...
func (d *PullHandlers) HandlePullChapter(ctx context.Context, job *Job) error {
	...
	for _, pg := range pages {
		if err := d.enqueueChild(ctx, JobTypePullPage, PullPagePayload{
			MangaID:    p.MangaID,
			ProviderID: p.ProviderID,
...
func (d *PullHandlers) enqueueChild(ctx context.Context, jobType string, payload any, group string, meta map[string]string) error {
