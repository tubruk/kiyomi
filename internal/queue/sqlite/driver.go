...
type Driver struct {
	...
	cleanupOpts     queue.CleanupOptions
}
...
func NewDriver(dbPath string, maxConcurrency int) (*Driver, error) {
	...
	// Auto-migrate: check if parent_id column exists
	rows, err := db.Query("PRAGMA table_info(jobs);")
	if err != nil {
		_ = db.Close()
		return nil, fmt.Errorf("inspect table info: %w", err)
	}
	hasParentID := false
	for rows.Next() {
		var cid int
	...
	if !hasParentID {
		if _, err := db.Exec("ALTER TABLE jobs ADD COLUMN parent_id TEXT;"); err != nil {
			_ = db.Close()
			return nil, fmt.Errorf("alter table add parent_id: %w", err)
		}
	}

	if _, err := db.Exec("CREATE INDEX IF NOT EXISTS idx_jobs_parent_id ON jobs(parent_id);"); err != nil {
		_ = db.Close()
		return nil, fmt.Errorf("create index idx_jobs_parent_id: %w", err)
	}
...
// SetCleanupConfig sets the interval and options for periodic automatic job cleanup.
func (d *Driver) SetCleanupConfig(interval time.Duration, opts queue.CleanupOptions) {
	d.activeMu.Lock()
	defer d.activeMu.Unlock()
...
func (d *Driver) CreateJob(ctx context.Context, job *queue.Job) error {
	...
	_, err = tx.ExecContext(ctx,
		`INSERT INTO jobs (id, parent_id, type, payload, status, retries, max_retries, concurrency_group, error, created_at, updated_at, started_at, completed_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		job.ID, job.ParentID, job.Type, job.Payload, string(job.Status), job.Retries, job.MaxRetries, job.ConcurrencyGroup,
...
func (d *Driver) GetJob(ctx context.Context, id string) (*queue.Job, error) {
	row := d.db.QueryRowContext(ctx,
		`SELECT id, parent_id, type, payload, status, retries, max_retries, concurrency_group, error, created_at, updated_at, started_at, completed_at,
		        (SELECT COUNT(*) FROM jobs c WHERE c.parent_id = jobs.id) AS child_count
	...
	err := row.Scan(&j.ID, &parentIDStr, &j.Type, &j.Payload, &statusStr, &j.Retries, &j.MaxRetries, &j.ConcurrencyGroup,
		&errorStr, &createdAtStr, &updatedAtStr, &startedAtStr, &completedAtStr, &childCount)
...
func (d *Driver) UpdateJob(ctx context.Context, job *queue.Job) error {
	...
	res, err := tx.ExecContext(ctx,
		`UPDATE jobs SET parent_id = ?, type = ?, payload = ?, status = ?, retries = ?, max_retries = ?, concurrency_group = ?, error = ?, 
		                created_at = ?, updated_at = ?, started_at = ?, completed_at = ?
		 WHERE id = ?`,
		job.ParentID, job.Type, job.Payload, string(job.Status), job.Retries, job.MaxRetries, job.ConcurrencyGroup, job.Error,
...
// DeleteJob deletes a job and its descendant child jobs from the database.
func (d *Driver) DeleteJob(ctx context.Context, id string) error {
	query := `
	WITH RECURSIVE job_tree(id) AS (
		SELECT id FROM jobs WHERE id = ?
		UNION ALL
		SELECT j.id FROM jobs j JOIN job_tree jt ON j.parent_id = jt.id
	)
	DELETE FROM jobs WHERE id IN (SELECT id FROM job_tree);
	`
...
func (d *Driver) ListJobs(ctx context.Context, filter queue.JobFilter) ([]*queue.Job, error) {
	query := `SELECT j.id, j.parent_id, j.type, j.payload, j.status, j.retries, j.max_retries, j.concurrency_group, j.error, j.created_at, j.updated_at, j.started_at, j.completed_at,
	                 (SELECT COUNT(*) FROM jobs c WHERE c.parent_id = j.id) AS child_count
	...
	if filter.ParentID != nil {
		...
		query += ` AND (j.parent_id IS NULL OR j.parent_id = '')`
	}
	...
	for rows.Next() {
		...
		err := rows.Scan(&j.ID, &parentIDStr, &j.Type, &j.Payload, &statusStr, &j.Retries, &j.MaxRetries, &j.ConcurrencyGroup,
			&errorStr, &createdAtStr, &updatedAtStr, &startedAtStr, &completedAtStr, &childCount)
		if err != nil {
			return nil, err
		}
...
// CancelJob cancels a job and recursively cancels all of its pending and running child jobs.
func (d *Driver) CancelJob(ctx context.Context, id string) error {
	var exists bool
	err := d.db.QueryRowContext(ctx, `SELECT EXISTS(SELECT 1 FROM jobs WHERE id = ?)`, id).Scan(&exists)
	if err != nil {
		return err
	}
	if !exists {
	...
	UPDATE jobs SET status = 'failed', error = 'job cancelled by user', updated_at = ?, completed_at = ?
	WHERE id IN (SELECT id FROM job_tree) AND status IN ('pending', 'running');
	`
	if _, err := d.db.ExecContext(ctx, query, id, nowStr, nowStr); err != nil {
		return fmt.Errorf("cancel job: %w", err)
	}
	return nil
}

// CleanupJobs prunes expired completed and failed jobs from the SQLite database.
func (d *Driver) CleanupJobs(ctx context.Context, opts queue.CleanupOptions) (int, error) {
	...
	completedCutoffStr := formatTime(&completedCutoff)
	failedCutoffStr := formatTime(&failedCutoff)

	candidateQuery := `
	WITH RECURSIVE subtree(root_id, job_id, status, completed_at, updated_at) AS (
		SELECT id, id, status, completed_at, updated_at
		FROM jobs
		WHERE parent_id IS NULL OR parent_id = ''
		UNION ALL
		SELECT s.root_id, j.id, j.status, j.completed_at, j.updated_at
		FROM jobs j
		JOIN subtree s ON j.parent_id = s.job_id
	)
	SELECT root_id
	FROM subtree
	GROUP BY root_id
	HAVING SUM(CASE WHEN status IN ('pending', 'running') THEN 1 ELSE 0 END) = 0
	   AND (
	       (SUM(CASE WHEN status = 'failed' THEN 1 ELSE 0 END) = 0 AND MAX(COALESCE(completed_at, updated_at)) <= ?)
	       OR
	       (SUM(CASE WHEN status = 'failed' THEN 1 ELSE 0 END) > 0 AND MAX(COALESCE(completed_at, updated_at)) <= ?)
	   )
	LIMIT ?;
	...
	rows, err := d.db.QueryContext(ctx, candidateQuery, completedCutoffStr, failedCutoffStr, opts.BatchSize)
	if err != nil {
		return 0, fmt.Errorf("find cleanup candidates: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&rootID); err != nil {
			return 0, fmt.Errorf("scan cleanup root id: %w", err)
		}
	...
	`)
	if err != nil {
		return 0, fmt.Errorf("prepare delete statement: %w", err)
	}
	defer deleteStmt.Close()

	prunedCount := 0
	for _, rootID := range rootIDs {
		res, err := deleteStmt.ExecContext(ctx, rootID)
		if err != nil {
			return prunedCount, fmt.Errorf("delete job tree %s: %w", rootID, err)
		}
		affected, err := res.RowsAffected()
		if err != nil {
			return prunedCount, fmt.Errorf("rows affected for %s: %w", rootID, err)
		}
		if affected > 0 {
			prunedCount++
		}
...
func (d *Driver) runJob(j *queue.Job) {
	...
	d.handlersMu.RUnlock()
	if !ok {
		execErr = fmt.Errorf("no handler registered for job type %s", j.Type)
		return
	}
	...
	if execErr != nil {
		...
		if permanent || j.Retries >= j.MaxRetries {
			_, _ = d.db.ExecContext(ctx,
				`UPDATE jobs SET status = ?, retries = ?, error = ?, updated_at = ?, completed_at = ? WHERE id = ? AND status = 'running'`,
				string(queue.StatusFailed), j.Retries, execErr.Error(), formatTime(&now), formatTime(&now), j.ID,
			)
		} else {
			_, _ = d.db.ExecContext(ctx,
				`UPDATE jobs SET status = ?, retries = ?, updated_at = ? WHERE id = ? AND status = 'running'`,
				string(queue.StatusPending), j.Retries, formatTime(&now), j.ID,
			)
		}
	} else {
		_, _ = d.db.ExecContext(ctx,
			`UPDATE jobs SET status = ?, updated_at = ?, completed_at = ? WHERE id = ? AND status = 'running'`,
